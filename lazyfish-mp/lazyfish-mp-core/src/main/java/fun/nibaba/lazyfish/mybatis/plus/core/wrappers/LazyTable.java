 ...
 */
@Getter
public class LazyTable<TableModel> implements Serializable {
     ...
     */
    private static final Map<String, Map<String, ColumnCache>> CLASS_COLUMN_MAP = new ConcurrentHashMap<>();
    ...
    private void tryInitCache(Class<?> clazz) {
        if (!CLASS_COLUMN_MAP.containsKey(clazz.getName())) {
            Map<String, ColumnCache> columnMap = LambdaUtils.getColumnMap(clazz);
            CLASS_COLUMN_MAP.put(clazz.getName(), columnMap);
        }
    }
    ...
    private ColumnCache getColumnCache(String fieldName, Class<?> clazz) {
        Map<String, ColumnCache> columnMap = CLASS_COLUMN_MAP.get(clazz.getName());
    ...
    private <T> SerializedLambda getSerializedLambda(SFunction<T, ?> column) {
        ...
        try {
            ...
            throw new LazyMybatisPlusException(e);
        }
